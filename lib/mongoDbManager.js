...
function setLastStatus(projectName, newStatus, callback) {
    db.collection(constants.PROJECTS_COLNAME, function(err, col) {
        if (err) {
            return callback(err);
        }
...
function getLastStatus(projectName, callback) {
    db.collection(constants.PROJECTS_COLNAME, function(err, col) {
        if (err) {
            return callback(err);
        }
        col.findOne({
            ...
            console.log('lastTestResult is ', project.lastTestResult); 
            return callback(null, project.lastTestResult);
        });
    });
}
